...
exports.getLinkList = (req, res) => {
  ...
  let type = Number(req.query.type);  // 1: 其它友情链接 2: 是博主的个人链接, '' 代表所有链接
...
exports.addLink = (req, res) => {
  ...
  Link.findOne({ name }).then(result => {
    if (!result) {
      let link = new Link({ name, desc, icon, url, type });
      link.save().then(data => {
      ...
      responseClient(res, 200, 1, '该链接名已经存在!');
    }
  }).catch(err => {
    responseClient(res);
  });
...
exports.delLink = (req, res) => {
  let { id } = req.body;
  Link.deleteMany({ _id: id }).then(result => {
    if (result.n === 1) {
